...
class DraftAgent(nn.Module):
    ...
    """
    def __init__(self, n_players_window=24, player_feat_dim=6, roster_feat_dim=48, embed_dim=64, num_heads=4):
        """
        :param n_players_window: Number of top available players to consider (N).
        :param player_feat_dim: Number of input features per player.
                                Default 6: [VOR, Value, Pos_QB, Pos_RB, Pos_WR, Pos_TE]
        :param roster_feat_dim: Number of input features for the roster context.
                                Default 48 (for 12 team league):
                                - 4 features for My Team
                                - 44 features for 11 Opponents (ordered relative to draft position)
        ...
        # 1. Roster/Context Encoder
        # Input: Raw player counts of my team and opponent teams.
        # Output: A "Query" vector representing the team's specific needs and the draft landscape.
        self.roster_encoder = nn.Sequential(
            nn.Linear(roster_feat_dim, embed_dim),
            nn.ReLU(),
            nn.Linear(embed_dim, embed_dim)
        )

        # 2. Player Board Encoder
    ...
    def forward(self, roster_features, player_features, mask=None):
        """
        ...
        roster_emb = self.roster_encoder(roster_features).unsqueeze(1)
